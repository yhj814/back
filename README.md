...
🚀해결 방법🚀<br>
쿼리문에서 신고 게시글을 조회할때 where절로 가져오는데 신고되지 않는 기본글들은 신고 id를 NULL로 지정하여 문제를 해결했다. <br><br>

<h3>6-4. 총평</h3>
<h4>🌟 기획: 기획이 정말 중요하다!  </h4>
프로젝트 기간이 길지 않다보니 기획을 나름 팀원들과 잘 기획했다고 생각했지만 중간중간 변경사항도 많았고, ERD도 수정해야하는 상황이 많았다. 또한 프론트작업을 할때 서버구현 생각을 하지 않고 화면을 구성한 부분이 있어서
서버작업을 할때 화면작업을 하고 있는 경우도 있었다. 하지만 이전에 했던 프로젝트를 생각하며 페이지 이동보다는 모달창을 띄우는 작업같이 사용자의 편의를 생각하여 기능을 만들었지만, 프로젝트를 진행하다보니, 사용자의 편의를 위한 작업은 끝이 없다는 것을 깨닫게 되었다.
요번 프로젝트를 통해 '완성' 하는 것이 아니라 계속해서 발전해나가고 수정하는 것이며, 왜 개발자들이 유지 보수를 위한 코드를 강조했는지 깨닫게 되었다.

<h4>🌟 협업: 프로젝트는 '나'만 하는 것이 아니라 '팀'과 함께 하는 것   </h4>
이번 프로젝트에서 느낀점은 내 작업도 중요하지만 팀원들의 작업 또한 중요하다는 것을 깨달았다. 내가 맡은 관리자페이지 신고쪽은 신고분야도 다양하고 페이지 부분을 많이 차지하고 있기 때문에 신고하기가 구현이 되지 않는다면 내 작업은 무의미한 작업이 되는 것이었다. 
결국, 팀원이 맡은 신고하기 부분은 구현되지 않았지만, 내가 임의로 신고테이블에 값을 넣어 일단 더미데이터들로 구현하여 마무리 되었다. 또한, 맡은 서비스가 겹칠경우, 서비스에서 사용하는 메소드 이름이나 DTO 등 역시 중요하다는 것을 알게되었다. 코드를 서로
공유하기 때문에 한쪽에서 보고 없이 임의로 수정을 하게 되면 상대방 코드에도 영향을 미치며 오류를 발생시키고 작업시간도 점점 지연되기 때문이다. 이 부분은 Slack과 주기적인 PR을 통해 어느부분 해결을 했지만 초반에 merge중에 충돌이 많이 일어나 코드를 고치는데 시간을 많이 잡아먹기도 했다.
이번 프로젝트를 통해 나의 작업은 곧 다른 팀원에게 영향을 미친다는 것을 깨닫게 해주었다.

<h4>🌟 미래: 발전해 나가는 개발자! </h4>
이번 프로젝트를 시작하며 나는 새로운 서비스를 맡아 보고 싶었고 지난 프로젝트에는 회원가입, 로그인 부분을 맡아서 진행했었다. 요번에는 다행히 다른 분야인 관리자 페이지 부분을 맡아 진행할 수 있었고 저번 프로젝트 때는 진행해보지 못했던 페이징처리와 검색 필터 부분을 처음 할떄는
어떻게 시작해야 할지도 모르겠고 막막했지만, 코드를 계속 찾아보고 출력해보고 수정한 결과 내가 맡은 분야는 성공적으로 끝낼 수 있어서 정말 뿌듯했다. 지금 생각해보면 그렇게 오래 잡고 있지 않아도 할 수 있는 부분이지만 그때가 있었기에 지금 처럼 생각할 수 있지 않나라고 느꼈고 이번일을 계기로
앞으로 나는 점점 발전해 나가는 개발자가 될 것이다.
