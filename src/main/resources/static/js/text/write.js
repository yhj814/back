let uploadedFileIds = []; // 일반 파일 ID 목록
let uploadedThumbnailFileId = null; // 썸네일 파일 ID
document.addEventListener("DOMContentLoaded", function () {
    ...
    //
    //                         // 파일 ID를 imgBox의 dataset에 저장
    //                         imgBox.dataset.fileId = fileId;
    //                     } else {
    ...
    submitButton.addEventListener("click", function (event) {
        ...
        const genreType = document.querySelector('input[name="genreType"]:checked').value;
        const workPrice = document.querySelector('input[name="workPrice"]').value;
        const fileContent = document.querySelector('input[name="fileContent"]').value;
        // WorkDTO 객체 생성
        const workDTO = {
            postTitle: postTitle,
            ...
            thumbnailFileId: uploadedThumbnailFileId
        };
        ...
        <div class="img-content-box">
            ...
            <div class="center-text img-box">
                <div class="default-img" id="default-img-${timestamp}">
                    ...
                    <input id="file-upload-${timestamp}" type="file" name="Files" accept="image/*,video/*" style="display: none;" />
                </div>
            </div>
        </div>
    `;
...
function previewImage(event) {
    ...
    if (file) {
        ...
        fetch('/text/upload', {
            ...
            .then(data => {
                if (data && data.length > 0) {
                    console.log('썸네일 파일이 성공적으로 업로드되었습니다.');
                    uploadedThumbnailFileId = data[0].id; // 썸네일 파일 ID 저장
                } else {
                    console.error('썸네일 파일 업로드 중 오류가 발생했습니다.');
                }
            })
            .catch(error => {
                console.error('Error:', error);
