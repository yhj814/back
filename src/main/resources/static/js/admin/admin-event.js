...
function setupVideoReplyReportModal() {
    ...
    saveButton.addEventListener("click", async () => {
        if (selectedReportReplyStatus) {
            ...
            if (success) {
                modal.style.display = "none";
                updateReplyReportStatusInView(selectedReplyId, selectedReportReplyStatus);
            }
        } else {
            alert("상태를 선택해주세요.");
        }
...
// 화면 신고 상태 업데이트
function updateReplyReportStatusInView(replyId, newStatus) {
...
function setupVideoReplyReportDetailsModal() {
    ...
    // 모든 "보기" 버튼에 이벤트 리스너 추가
    document.querySelectorAll(".report-content-look-reply").forEach(button => {
        button.addEventListener("click", () => {
            console.log("보기 버튼 클릭"); // 로그 추가
...
function setupVideoReplyContentClickEvents() {
    document.querySelectorAll(".apply-table-row .apply-table-cell:nth-child(4)").forEach(cell => {
        cell.addEventListener("click", (e) => {
            ...
            openReplyModal(replyContent);
        });
    });
}

// 댓글 모달 열기 함수
function openReplyModal(replyContent) {
    const replyModal = document.getElementById("reply-detail-modal");
    const replyContentTextarea = document.getElementById("reply-content-textarea");
    replyContentTextarea.value = replyContent;
    replyModal.style.display = "flex";
}

// 모달 외부 클릭 시 닫기
document.getElementById("modal-reply-overlay").addEventListener("click", () => {
