...
let emailTimeRemaining = 10; // 임의로 10초 타이머
    ...
    if (validateField(emailField, emailRegex, "올바른 이메일 주소를 입력해주세요.")) {
        ...
        try {
            ...
            if (response.ok) {
                ...
                document.querySelector(".email-certification-input").classList.remove("certification-input"); // 이메일 인증 필드 표시
...
function startEmailTimer() {
    emailTimeRemaining = 10; // 초기화 후 10초로 설정
    ...
    emailTimerInterval = setInterval(() => {
        ...
        if (emailTimeRemaining <= 0) {
            ...
            timerDisplay.style.color = "red"; // 만료 메시지 색상 변경
